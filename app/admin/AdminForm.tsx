...
export default function AdminForm() {
    ...
    const addComponent = (type: 'text' | 'button' | 'image' | 'link') => {
    ...
    const addPresetButton = (preset: 'claim' | 'referral') => {
        const newComponent: EmailComponent = {
            ...
            content: preset === 'claim' ? 'Claim Your Username' : 'Get Your Referral Link',
            preset: preset,
        };
        setComponents([...components, newComponent]);
    };

    const updateComponent = (id: number, field: keyof EmailComponent, value: string) => {
        setComponents(
            components.map((comp) =>
    ...
    return (
        <>
            <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '20px', maxWidth: '700px' }}>
                ...
                <fieldset style={{ border: '1px solid #0070f3', padding: '15px', borderRadius: '5px', marginBottom: '20px' }}>
                    <legend>Template Management</legend>
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr auto', gap: '10px', marginBottom: '15px' }}>
                        <select value={selectedTemplate} onChange={(e) => handleLoadTemplate(e.target.value)} style={{ width: '100%', padding: '8px' }}>
                            <option value="">-- Load a Saved Template --</option>
                            {templateList.map(filename => (
                                <option key={filename} value={filename}>{filename.replace('.json', '')}</option>
                            ))}
                        </select>
                        <button type="button" onClick={resetForm}>+ New Template</button>
                    </div>
                ...
                <label className="flex flex-col">
                    Target Audience:
                    <select value={targetGroup} onChange={(e) => setTargetGroup(e.target.value)} className="px-4 py-2 border rounded-md w-full">
                        <option value="all">All Users</option>
                        <option value="claimed_username">Created a Username</option>
                        <option value="not_claimed_username">Did Not Create a Username</option>
                        <option value="not_referred_anyone">Users Who Haven't Referred Anyone</option>
              ...
              <div>
                ...
                <fieldset style={{ border: '1px solid #ccc', padding: '15px', borderRadius: '5px' }}>
                    ...
                    <input type="text" placeholder="Email Subject" value={subject} onChange={(e) => setSubject(e.target.value)} required style={{ padding: '8px', width: '100%' }} />
                    <input type="url" placeholder="Optional Header Image URL" value={imageUrl} onChange={(e) => setImageUrl(e.target.value)} style={{ padding: '8px', width: '100%', marginTop: '10px' }} />
                </fieldset>

                {/* Component Builder UI */}
                <div id="component-builder">
                    {components.map((comp, index) => (
                        <div key={comp.id} style={{ border: '1px solid #ccc', padding: '15px', borderRadius: '5px', marginBottom: '15px', position: 'relative' }}>
                            <button type="button" onClick={() => removeComponent(comp.id)} style={{ position: 'absolute', top: '5px', right: '5px', background: 'red', color: 'white', border: 'none', borderRadius: '50%', width: '20px', height: '20px', cursor: 'pointer', lineHeight: '20px', textAlign: 'center' }}>X</button>
                            <strong style={{ textTransform: 'capitalize' }}>{comp.preset ? `${comp.preset} Button` : comp.type}</strong>
                            {comp.type === 'text' ? (
                                <>
                                    <textarea
                                        value={comp.content}
                                        onChange={(e) => updateComponent(comp.id, 'content', e.target.value)}
                                        style={{ minHeight: '100px', width: '100%', padding: '8px', marginTop: '5px' }}
                                    />
                                    <div style={{ marginTop: '5px' }}>
                                        <button
                                            type="button"
                                            onClick={() => updateComponent(comp.id, 'content', comp.content + ' {name}')}
                                            style={{ background: '#e0e0e0', border: '1px solid #ccc', padding: '4px 8px', borderRadius: '4px', cursor: 'pointer' }}
                                        >
                                            + Insert User's Name
                                        </button>
                                    </div>
                                ...
                                <div style={{ marginTop: '5px' }}>
                                    ...
                                    <input
                                        ...
                                        value={comp.link}
                                        onChange={(e) => updateComponent(comp.id, 'link', e.target.value)}
                                        disabled={!!comp.preset} // Disable link input for presets
                                        style={{ width: '100%', padding: '8px', background: comp.preset ? '#f0f0f0' : 'white' }}
                                    />
                                </div>
                            )}
                        </div>
                ...
                <div style={{ display: 'flex', gap: '10px', borderTop: '2px solid #eee', paddingTop: '15px' }}>
                    <button type="button" onClick={() => addComponent('text')}>+ Add Text</button>
                ...
                {sendTestUser ? <button type="submit" style={{ padding: '12px', background: 'black', color: 'white', border: 'none', cursor: 'pointer', fontSize: '16px' }}>Send Test Email</button> : <button type="submit" style={{ padding: '12px', background: 'black', color: 'white', border: 'none', cursor: 'pointer', fontSize: '16px' }}>Send Emails</button>}
            </form>
